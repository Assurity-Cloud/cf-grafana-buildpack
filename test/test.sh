...
EOF
  local key_location=$(create_ca_cert "${service}" mydb tmp)
  processExitCode=$?
  assertEquals 0 "${processExitCode}"
  assertEquals "tmp/mydb-ca.crt" "${key_location}"
...
EOF
  local cert_location=$(create_client_cert "${service}" mydb tmp)
  processExitCode=$?
  assertEquals 0 "${processExitCode}"
  assertEquals "tmp/mydb-client.crt" "${cert_location}"
...
EOF
  local key_location=$(create_client_key "${service}" mydb tmp)
  processExitCode=$?
  assertEquals 0 "${processExitCode}"
  assertEquals "tmp/mydb-client.key" "${key_location}"
